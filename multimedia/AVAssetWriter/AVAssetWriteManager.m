...
- (void)setUpInitWithType:(FMVideoViewType )type
{
    ...
    switch (type) {
        case Type1X1:
            _outputSize = CGSizeMake(bounds.size.width, bounds.size.width);
            break;
        case Type4X3:
            _outputSize = CGSizeMake(bounds.size.width, bounds.size.width*4/3);
            break;
        case TypeFullScreen:
            _outputSize = CGSizeMake(bounds.size.width, bounds.size.height);
            break;
        default:
            _outputSize = CGSizeMake(bounds.size.width, bounds.size.width);
            break;
...
//    NSLog(@"写入宽度 %f 写入高度 %f",self.outputSize.width,self.outputSize.height);
    //每像素比特
//    CGFloat bitsPerPixel = 6.0;
//    NSInteger bitsPerSecond = numPixels * bitsPerPixel;//1828224
    
    // 码率和帧率设置                           AVVideoAverageBitRateKey : @(bitsPerSecond),
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSInteger selectDPI  = [userDefaults integerForKey:@"Encode_DPI"];
    NSInteger selectFPS  = [userDefaults integerForKey:@"Encode_FPS"] +  15;//15-25
    NSInteger selectCR   = [userDefaults integerForKey:@"Encode_CodeRate"];
    
    NSInteger width,height,codeRate;
    if (selectDPI == 0) {
        width = 640;
        height = 480;
        codeRate = selectCR == 0 ? 500 : (selectCR == 1 ? 1000 : 2000);
    }else if (selectDPI == 1){
        width = 1280;
        height = 720;
        codeRate = selectCR == 0 ? 1000 : (selectCR == 1 ? 2500 : 4000);
    }else {
        width = 1920;
        height = 1080;
        codeRate = selectCR == 0 ? 2000 : (selectCR == 1 ? 4000 : 8000);
    }
    
    //分辨率
    NSInteger bitsPerSecond = codeRate * 1024;
    NSDictionary *compressionProperties = @{ AVVideoAverageBitRateKey : @(bitsPerSecond),
                                             AVVideoExpectedSourceFrameRateKey : @(selectFPS),
                                             AVVideoMaxKeyFrameIntervalKey : @(selectFPS),
...
//                                       AVSampleRateKey : @(22050) };
    self.audioCompressionSettings = @{ AVSampleRateKey : @(8000),
                                       AVFormatIDKey : @(kAudioFormatMPEG4AAC),
