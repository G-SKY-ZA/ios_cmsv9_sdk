...
- (instancetype)initWithFMVideoViewType:(FMVideoViewType)type superView:(UIView *)superView videoName:(NSString *)videoName
{
    self = [super init];
    if (self) {
...
//    [self.videoOutput recommendedVideoSettingsForAssetWriterWithOutputFileType: nil];
    

    if([videoCaptureDevice lockForConfiguration:nil])//    请求独占访问以配置设备硬件属性
    {
        NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
        NSInteger selectFPS  = [userDefaults integerForKey:@"Encode_FPS"] +  15;//15-25
    [videoCaptureDevice setActiveVideoMaxFrameDuration:CMTimeMake(1, selectFPS)];
        [videoCaptureDevice setActiveVideoMinFrameDuration:CMTimeMake(1, selectFPS)];
...
- (void)setUpPreviewLayerWithType:(FMVideoViewType )type
{
    ...
    switch (type) {
        case Type1X1:
            rect = CGRectMake(0, 0, bounds.size.width, bounds.size.width);
            break;
        case Type4X3:
            rect = CGRectMake(0, 0, bounds.size.width, bounds.size.width*4/3);
            break;
        case TypeFullScreen:
            rect = bounds;
            break;
...
{
    NSString *cacheDir = [XCFileManager documentsDir];
...
- (NSString *)createVideoFilePath
{
    ...
    if (_videoName) {
        videoName = [NSString stringWithFormat:@"%@.mp4", _videoName];
    }else{
    videoName = [NSString stringWithFormat:@"%@.mp4", [NSUUID UUID].UUIDString];
    }
